...
def check_name_state(name):
    ...
    rname=nm[1]
    fname=nm[0]
    course=databaseScript.check_course(rname)
    lis=list(course.split(","))
    atn=databaseScript.verify()
    now=datetime.now()
    d1=now.strftime("%d/%m/%Y")
    if(not databaseScript.exist_name(fname,d1)) :
        if atn in lis:
            dtstring=now.strftime("%d/%m/%Y %H:%M:%S")
            databaseScript.insert_data(rname,fname,atn,dtstring)
            print("inserted")
